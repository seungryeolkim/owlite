...
class Signature(list[tuple[str, tuple[int, ...]]]):
    ...
    def from_module(
        ...
        if isinstance(module, GraphModule) and isinstance(original_params := module.meta.get("original_params"), dict):
            signature_map = map_graph_module_signature(module, original_params, *args, **kwargs)
        else:
...
def map_graph_module_signature(
    module: GraphModule,
    original_params: dict[str, inspect.Parameter],
    *args: Any,
    **kwargs: Any,
) -> dict[str, Any]:
    """Maps the args and kwargs to the parameters of the forward method of a graph module
    generated by `owlite.fx.symbolic_trace`

    Args:
        module (GraphModule): a graph module generated by `owlite.fx.symbolic_trace`
        original_params (dict[str, inspect.Parameter]): the original parameters. It must be the same object as
            `module.meta["original_params"]`
        args (Any): Positional arguments.
        kwargs (Any): Keyword arguments.
    ...
    """
    assert original_params is module.meta.get("original_params", None)
    ...
    for p, (k, v) in enumerate(mapped_signature.items()):
        ...
        if isinstance(v, tuple):
            ...
            # when two arguments are provided by the user
        ...
        if isinstance(v, dict):
            ...
            for name, x in v.items():
                if name in modified_params:
                    signature_map[name] = x
            continue
        if f"args_{p}" in modified_params:
